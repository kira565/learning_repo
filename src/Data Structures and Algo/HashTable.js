...
const getCity = (year) => capitalsHash[Math.floor(year / 20)];
...
//Коллизия
const getCity3 = (year) => capitals3[year % 100];
...
const capitalsHashWithoutColissius = Array(100);
function setCapital(year, city) {
    const index = year % capitalsHashWithoutColissius.length;
    if (!(index in capitalsHashWithoutColissius)) {
        capitalsHashWithoutColissius[index] = new SinglyLinkedList_1.LinkedListImpl();
    }
    if (index in capitalsHashWithoutColissius) {
...
class HashTable {
    ...
    set(key, value) {
        ...
        if (!(idx in this.table)) {
            this.table[idx] = new SinglyLinkedList_1.LinkedListImpl();
        }
    ...
    get(key) {
        ...
        if (hash in this.table) {
            ...
            const value = list.search((val) => val.name === key);
            return value;
        }
        return undefined;
    }
