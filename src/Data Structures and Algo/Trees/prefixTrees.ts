...
class Trie {
  ...
  parent: Trie | null;
  end: boolean; // terminal characteristic (терминальность)

  constructor(key: string, parent: Trie | null) {
    this.key = key;
  ...
  remove(word: string) {
    let node: Trie = this;
    const findWord = (node: Trie, word: string, index: number) => {
